---
title: Lab 5 - Student Evaluations
...
```{r}
#| label: explore-data
# This dataset provides information about the student survey responses, student success in the course, and breakdown of the course schedule. In regards to the student surveys, the data provides average scores, number of participants, and share of responses. For student success, the dataset possesses variables like grade_avg, grade_std, and percent_failed. To breakdown the metrics of the course to evaluate why students may respond the way they did, the dataset has variables for duration of the class, time of day and days of the week they were given. 
...
teacher_evals_clean <- read_csv("data-clean/teacher_evals_clean.csv")
```
...
teacher_evals_clean |>
  n_distinct("teacher_id", "course_id", na.rm = TRUE)
```
...
# code chunk for Q6
# was confused on how to properly utilize if_any() - used chatgpt to see what code to add to provide a result (everything())
#PAB3SE004PA, teacher id 56347, NAs in percent_failed
...
# code chunk for Q7
# Among the teachers in this dataset, 32 have doctorates, 19 have a masters degree, 6 have no degrees, 7 have the prof label. There are 23 female teachers and 31 male teachers. Seniority varies from 1 to 11 - with the most having 1 year. 

teacher_evals_clean |>
  select(academic_degree, seniority, sex) |>
  distinct(academic_degree, seniority, sex, .keep_all = TRUE) |>
...
teacher_evals_clean |>
  ...
  summarize(distinct_questions = n_distinct(question_no)) |>
  ungroup() |>
  filter(distinct_questions == 9)
```
...
teacher_evals_clean |>
  select(teacher_id, sex, resp_share, academic_degree) |>
  filter(sex == "female",
         academic_degree == c("dr", "prof")) |>
  arrange(desc(resp_share))
```
